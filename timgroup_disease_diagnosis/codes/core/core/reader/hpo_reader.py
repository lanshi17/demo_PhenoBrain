...
def resolve_hpo_raw_path(preferred_relative_path, fallback_relative_path):
	preferred_path = os.path.join(DATA_PATH, preferred_relative_path)
	fallback_path = os.path.join(DATA_PATH, fallback_relative_path)
...
class HPOReader(object):
	...
	def __init__(self):
		...
		self.HPO_OBO_PATH = resolve_hpo_raw_path(
			os.path.join('raw', 'HPO', '2019', 'Ontology', 'hp.obo'),
			os.path.join('raw', 'HPO', '2022', 'Ontology', 'hp.obo'),
		)
		...
		self.ANNOTATION_HPOA_PATH = resolve_hpo_raw_path(
			os.path.join('raw', 'HPO', '2019', 'Annotations', 'phenotype.hpoa'),
			os.path.join('raw', 'HPO', '2022', 'Annotations', 'phenotype.hpoa'),
		)
